...
public abstract class NmeaSentence implements INmeaSentence {
	...
	public String toString() {
		...
		List<Field> sentenceFields = AbstractNmeaSentenceCodec
				.getSentenceFields(this);

		try {
			for (Field field : sentenceFields) {
				SentenceField annotation = (SentenceField) field
						.getAnnotation(SentenceField.class);
			...
			List<Field> messageFields = AbstractNmeaSentenceCodec
					.getMessageFields(this);
			...
			if (this instanceof EncapsulationSentence) {
				IEncapsulatedSentence encapsulated = ((EncapsulationSentence) this)
						.getEncapsulatedSentence();

				if (encapsulated != null) {
					List<Field> embeddedFields = AbstractNmeaSentenceCodec
							.getMessageFields(encapsulated);
