...
public class RmcNmeaObject extends AbstractNmeaObject{
	...
	public String toString() {
		...
		if(!this.msgFields.get(2).isEmpty()) {
			str.append(",定位状态:");
		...
		if(!this.msgFields.get(4).isEmpty()) {
		...
		if(!this.msgFields.get(6).isEmpty()) {
		...
		if(!this.msgFields.get(5).isEmpty()) {
		...
		if(!this.msgFields.get(10).isEmpty()) {
			str.append(",磁偏角:");
